...
# We want to replace the sections:
# 1. Client/Tenant Management Section
# 2. Financial Flow & Monitoring Section
# 3. Interactive Process / Workflow Section
# With our new PPT-based sections:
# 1. Pain Points & Solutions
# 2. Core Features (四大核心模块)
# 3. Ticket Workflow (客户工单闭环)
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-slate-50 rounded-3xl p-8 md:p-12 relative overflow-hidden border border-slate-100 reveal">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-8 text-center relative z-10">
                    <div class="p-6 bg-white rounded-2xl shadow-sm border border-slate-100 hover:shadow-xl transition-shadow duration-300">
                    ...
                    <div class="p-6 bg-white rounded-2xl shadow-sm border border-slate-100 hover:shadow-xl transition-shadow duration-300 relative">
...
translation_zh_update = """
                ...
                workflow: {
                    ...
                    s3_title: "3. 严谨审核批复", s3_desc: "财务核令人核对金额与要求后批复。审核金额、时间、操作人、备注信息永久记录入库，不可篡改。"
                },
"""
...
with open('d:/AI生成原型/yiyu_digital_media/update_home.py', 'w', encoding='utf-8') as f:
    f.write('''import re
import os
...
translation_zh_update = \"\"\"""" + translation_zh_update + "\"\"\"\n\n" + """
translation_en_update = \"\"\"""" + translation_en_update + "\"\"\"\n\n" + """
content = content.replace("hero: {", translation_zh_update + "hero: {", 1)
